...
namespace dai_nodes {
RGB_test::RGB_test(const std::string& daiNodeName,
         rclcpp::Node* node,
         std::shared_ptr<dai::Pipeline> pipeline,
         dai::CameraBoardSocket socket = dai::CameraBoardSocket::CAM_A,
         depthai_ros_driver::dai_nodes::sensor_helpers::ImageSensor sensor = {"IMX378", "4k", {"12mp", "4k"}, true},
         bool publish = true)
    : depthai_ros_driver::dai_nodes::BaseNode(daiNodeName, node, pipeline) {
...
RGB_test::~RGB_test() = default;

void RGB_test::setNames() {
    ispQName = getName() + "_isp";
    previewQName = getName() + "_preview";
    controlQName = getName() + "_control";
}

void RGB_test::setXinXout(std::shared_ptr<dai::Pipeline> pipeline) {
    if(ph->getParam<bool>("i_publish_topic")) {
        xoutColor = pipeline->create<dai::node::XLinkOut>();
        xoutColor->setStreamName(ispQName);
        if(ph->getParam<bool>("i_low_bandwidth")) {
            videoEnc = depthai_ros_driver::dai_nodes::sensor_helpers::createEncoder(pipeline, ph->getParam<int>("i_low_bandwidth_quality"));
...
void RGB_test::setupQueues(std::shared_ptr<dai::Device> device) {
    if(ph->getParam<bool>("i_publish_topic")) {
        auto tfPrefix = getTFPrefix(depthai_ros_driver::utils::getSocketName(static_cast<dai::CameraBoardSocket>(ph->getParam<int>("i_board_socket_id"))));
        ...
        if(ph->getParam<std::string>("i_calibration_file").empty()) {
            infoManager->setCameraInfo(depthai_ros_driver::dai_nodes::sensor_helpers::getCalibInfo(getROSNode()->get_logger(),
                                                                    *imageConverter,
                                                                    device,
        ...
        if(ipcEnabled()) {
            ...
            colorQ->addCallback(std::bind(depthai_ros_driver::dai_nodes::sensor_helpers::splitPub,
                                          std::placeholders::_1,
                                          std::placeholders::_2,
                                          *imageConverter,
                                          rgbPub,
            ...
            colorQ->addCallback(std::bind(depthai_ros_driver::dai_nodes::sensor_helpers::cameraPub,
                                          std::placeholders::_1,
                                          std::placeholders::_2,
                                          *imageConverter,
                                          rgbPubIT,
    ...
    if(ph->getParam<bool>("i_enable_preview")) {
        ...
        imageConverter->setUpdateRosBaseTimeOnToRosMsg(ph->getParam<bool>("i_update_ros_base_time_on_ros_msg"));
        if(ph->getParam<std::string>("i_calibration_file").empty()) {
            previewInfoManager->setCameraInfo(depthai_ros_driver::dai_nodes::sensor_helpers::getCalibInfo(getROSNode()->get_logger(),
                                                                           *imageConverter,
                                                                           device,
                                                                           static_cast<dai::CameraBoardSocket>(ph->getParam<int>("i_board_socket_id")),
        ...
        if(ipcEnabled()) {
            ...
            previewQ->addCallback(
                std::bind(depthai_ros_driver::dai_nodes::sensor_helpers::basicCameraPub, std::placeholders::_1, std::placeholders::_2, *imageConverter, previewPubIT, previewInfoManager));
        } else {
            ...
            previewQ->addCallback(std::bind(depthai_ros_driver::dai_nodes::sensor_helpers::splitPub,
                                            std::placeholders::_1,
                                            std::placeholders::_2,
                                            *imageConverter,
                                            previewPub,
...
void RGB_test::link(dai::Node::Input in, int linkType) {
    if(linkType == static_cast<int>(depthai_ros_driver::dai_nodes::link_types::RGBLinkType::video)) {
        colorCamNode->video.link(in);
    } else if(linkType == static_cast<int>(depthai_ros_driver::dai_nodes::link_types::RGBLinkType::isp)) {
        colorCamNode->isp.link(in);
    } else if(linkType == static_cast<int>(depthai_ros_driver::dai_nodes::link_types::RGBLinkType::preview)) {
...
void RGB_test::updateParams(const std::vector<rclcpp::Parameter>& params) {
